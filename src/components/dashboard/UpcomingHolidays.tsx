...
const typeTone: Record<Holiday['type'], string> = {
    ...
    <div className="rounded-2xl surface p-5">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-base font-semibold text-[var(--text-primary)]">Upcoming Holidays</h3>
            <p className="text-xs text-[var(--text-secondary)]">Next company & national breaks</p>
          </div>
        </div>
        <Link to="/holidays" className="text-xs font-medium text-brand-accent hover:underline">
          View all
        </Link>
      </div>
          ...
          return (
            <Link key={h.id} to="/holidays">
              ...
              <div className="flex flex-col items-end gap-1">
                <span className={cn('rounded-full px-2 py-0.5 text-[10px] font-semibold', typeTone[h.type])}>
                  {h.type}
                </span>
